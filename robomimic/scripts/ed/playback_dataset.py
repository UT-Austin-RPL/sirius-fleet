...
from robomimic.scripts.ed.error_detectors import SiriusFleet_Failure, SiriusFleet_OOD, SiriusFleetCombined, get_obs_at_idx
...
def playback_dataset(args):
    ...
    if args.fail_detector_path is not None:
        ...
        fail_detector = SiriusFleet_Failure(args.fail_detector_path, 0.8, 3, 2, "idx", eval_idx=slice(7,10), pred_future=args.pred_future, use_prob=True)
    if args.embedding_path is not None:
        global ood_detector
        ood_detector = SiriusFleet_OOD(args.ood_ckpt_path, None, args.embedding_path, pred_future=args.pred_future, eval_method="last", dist_metric=args.ood_type, train_kmeans=True)
        
    if fail_detector is not None and ood_detector is not None:
        global detector
        detector = SiriusFleetCombined(ood_detector, fail_detector)
